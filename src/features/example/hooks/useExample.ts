...
import { ExampleRepository } from '@/entities/example/api/example.repository';
...
export function useExampleItems() {
  const { data, isLoading, error, isError } = useApiQuery(EXAMPLE_KEYS.lists(), (signal) => ExampleRepository.getItems(signal));
