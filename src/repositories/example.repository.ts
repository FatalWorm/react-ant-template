import { API } from '@/api';
import type { TExampleItem } from '@/api/modules/example/example.types';
...
export class ExampleRepository {
  ...
  static async getItems(signal?: AbortSignal): Promise<TExampleItem[]> {
    const items = await API.examples.getItems(signal);
  ...
  static async getItemById(id: number, signal?: AbortSignal): Promise<TExampleItem> {
    return API.examples.getItemById(id, signal);
  }
  ...
  static async createItem(title: string): Promise<TExampleItem> {
    ...
    return API.examples.createItem({ 
      title, 
      completed: false 
    });
  }
